...
class MeshDrawer {
	...
	constructor() {
		 ...
		 */
		this.Positionofl = gl.getUniformLocation(this.prog, 'lightPos');
		this.ambiance = gl.getUniformLocation(this.prog, 'ambient');
		this.endisablelight = gl.getUniformLocation(this.prog, 'enableLighting');
	...
	draw(trans) {
		...
		gl.uniform3f(this.Positionofl, lightX, lightY, 1.0);
		/**
	...
	enableLighting(show) {
		 ...
		 */
	gl.useProgram(this.prog);
		gl.uniform1i(this.endisablelight, show ? 1 : 0);
	}
	...
	setAmbientLight(ambient) {
		 ...
		 */
	gl.useProgram(this.prog);
		gl.uniform1f(this.ambiance, ambient);
	}
}
...
const meshFS = `
			...
			void main()
			{
			vec3 vect = normalize(v_normal);
			
			
			vec3 Lightrat = normalize(lp - gl_FragCoord.xyz);
			
			float xs = max(dot(vect, Lightrat), 0.0);

			vec4 res = texture2D(tex, v_texCoord);
			vec4 dColor = vec4(xs, xs, xs, 1.0);
			vec4 Aambient = vec4(ambient, ambient, ambient, 1.0);

			vec4 litColor = res * (dColor + Aambient);	
